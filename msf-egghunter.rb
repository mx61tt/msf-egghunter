  ...
  class OptsConsole
    def self.parse(args)
      ...
      parser = OptionParser.new do |opt|
        opt.banner = "Usage: msf-egghunter [options]\nExample: msf-egghunter -f python -e W00T -ppr 41414141"
        opt.separator ''
        ...
        opt.on('-ppr', '--pop2ret <String>', "The POP; POP; RET (Please give 4 bytes)") do |v|
          options[:pop2ret] = v
        end
      ...
      if options.empty?
        ...
        raise OptionParser::MissingArgument, '-ppr is required'
      ...
      options[:arch]     = ARCH_X64  unless options[:arch]
